...
def evaluate_and_plot():
    ...
    MODEL_PATH = "early_fusion_mulb_optimized.pth"
    OUTPUT_IMAGE = "confusion_matrix_early.png"
    ...
    # 2. Load Model
    print(f"Loading Early Fusion model from {MODEL_PATH}...")
    ...
    with torch.no_grad():
        for hand_imgs, iris_imgs, labels in tqdm(dataloader):
            ...
            # Early Fusion forward expects two inputs, just like Late Fusion wrapper, 
            # effectively handling the concatenation inside.
    ...
    sns.heatmap(cm, annot=annotate, fmt='d', cmap='Oranges', # Using distinct color (Orange) for Early Fusion
                xticklabels=False, yticklabels=False)
    
    plt.title(f'Early Fusion Confusion Matrix (Accuracy: {acc:.2%})')
